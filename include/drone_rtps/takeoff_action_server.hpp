...
public:
  ...
  DRONE_PUBLIC explicit TakeoffActionServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
...
private:  
  ...
  void vehicle_command_takeoff(float altitude) const;
  void vehicle_command_arm() const;
  void publish_offboard_control_mode() const;
  bool read_position(float *x, float *y, float *z, float *w);
  void set_takeoff_altitude(float altitude);
